...
int main(int argc, char** argv)
{
  ...
  FILE* f = popen("/bin/ps -FA | /bin/grep 'nacl_helper_bootstrap_x86_64' | /bin/grep -oE '[0-9]+'","r");
  char* line = 0;
  size_t n = 0;
  if (getline(&line,&n,f) == -1)
  {
    exit(printf("nacl not running\n"));
  }
  pclose(f);
  char* end = 0;
  ...
  if (argc==3)
  {
    size_t start_r = strtoull(argv[1],&end,16);
    size_t end_r = strtoull(argv[2],&end,16);
    for(i=start_r; i<end_r; i+=4*1024)
    {
      printf("%20zx -> %20zx\n",i,vaddr2paddr(fp,i));
    }
